...
int cabbage_goat[2], goat_wolf[2],temp;
int farmer() {
	int i ;
	for ( i = 0 ; i < ROUNDS ; i ++ ) {
		farmer_in = 1 ; // Farmer notifies the world that he is in
		if ( write(goat_wolf[1],&temp,sizeof(temp)) < 0 ) { 
			perror(NULL) ;
			return 1 ;
		}
		if ( write(cabbage_goat[1],&temp,sizeof(temp)) < 0 ) { 
			perror(NULL) ;
			return 1 ;
		}
		...
		if ( read(cabbage_goat[0],&temp,sizeof(temp)) < 0 ) 
		{
			perror(NULL) ;
			return 1 ;
		}
		if ( read(goat_wolf[0],&temp,sizeof(temp)) < 0 ) 
		{
			perror(NULL) ;
			return 1 ;
		}
