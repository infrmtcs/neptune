#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
#include <sys/wait.h>
#include <errno.h>
int main(int argc, char **argv)
{

	int n = atoi(argv[1]) ; // read argument from command line
	int superdaddy = getpid() ;
	int p[2] ; // pipe
	int output, savedoutput, result1, result2, result, cpid, s1;
	s1 = 1;
	output = STDOUT_FILENO ;//this const cannot run in ubuntu, so i use printf for the corner case n = 1, n =2;
	if (n <= 2) 
	{
		printf("Result: 1\n") ;
		return 0;
	}
	while ( 1 )
	{ // every new child must go through the while body again
		if ( n <= 2)
		{ // base case
			if ( write(output,&s1,sizeof(s1)) < 0 ) {
				perror(NULL) ;
				return 1 ;
			}
			return 0 ;
		}
		savedoutput = output ; // useful when 2 parent spawns two children (pipe with parent and current)

		n -- ; // argument for child
		if ( pipe(p) < 0 )
		{ // build pipe to return result
			perror(NULL) ;
			return 1 ;
		}
		output = p[1] ; // useful when parent spawns 2 children (pipe will now be with current and its child)
		cpid = fork() ; // create child to compute F[n-1]
		if ( cpid == 0 )// child
		{
			if (close(p[0])<0)
			{ // close unused pipe ends
				perror(NULL) ;
				return 1 ;
			}
			continue ;
		}else if (cpid<0)
		{
			perror(NULL) ;
			return 1 ;
		}
		if (close(p[1])<0)
		{ // close unused pipe ends
			perror(NULL) ;
			return 1 ;
		}
		if ( read(p[0],&result1,sizeof(result1)) < 0 ) 
		{
			perror(NULL) ;
			return 1 ;
		}

		n -- ; // argument for child
		if ( pipe(p) < 0 )
		{ // build pipe to return result
			perror(NULL) ;
			return 1 ;
		}
		output = p[1] ; // useful when parent spawns 2 children (pipe will now be with current and its child)
		cpid = fork() ; // create child to compute F[n-2]
		if ( cpid == 0 )// child
		{
			if (close(p[0])<0)
			{ // close unused pipe ends
				perror(NULL) ;
				return 1 ;
			}
			continue ;
		}else if (cpid<0)
		{
			perror(NULL) ;
			return 1 ;
		}
		if (close(p[1])<0)
		{ // close unused pipe ends
			perror(NULL) ;
			return 1 ;
		}
		if ( read(p[0],&result2,sizeof(result2)) < 0 ) // co close ko nhi
		{
			perror(NULL) ;
			return 1 ;
		}
		break ;
	}
	result = result1 + result2;
	if ( getpid()==superdaddy ) // the original parent prints the result
		printf("Result: %d\n", result) ;
	else if ( write(savedoutput,&result,sizeof(result)) < 0 ) {
		// each process will send the computed partial result to its own parent
		perror(NULL) ;
		return 1 ;
	}
	if (close(savedoutput)<0)
	{ // close unused pipe ends
		perror(NULL) ;
		return 1 ;
	}
	wait(NULL) ; // wait for the child to terminate (avoid zombies)
	return 0 ;
}